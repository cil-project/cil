...
/*********** DEEP COPY WRAPPER. *******/
//the cilnoremove assures that these functions are still here when CCured
//builds the deep copier.
#pragma cilnoremove("__arrayofstrings_fatten");
#pragma cilnoremove("__arrayofstrings_lean");
...
struct bar __LEAN* copy_bars_to_lean(struct bar * src) {
  struct bar __LEAN* new_bars;
  int i;
  if (src == NULL)
    return NULL;

  new_bars = malloc(NR_BARS * sizeof(*new_bars));
  for (i = 0; i < NR_BARS; i++) {
    __deepcopy_to_lean_bar(&new_bars[i], &src[i]);
  }
  return __ptrof(new_bars);
}
#pragma cilnoremove("__arrayofstrings_fatten");
...
#pragma ccureddeepcopytolean("foo", "bars", "copy_bars_to_lean");
//generated by CCured:
