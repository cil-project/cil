...
of a variable or function must appear before its use in a file. 
 ...
 CIL supports pragmas only at the top level and pragmas share the rich
...
\cilvalref{alignOff_int}, and can convert an offset into a start and width
...
functions do not take into account the \t{packed} attributes and pragmas. 
 ...
 Each type can have a set of attributes (described in \secref{attrib}). They
are represented using the type \ciltypref{attribute}. An attribute consists of
a name and a number of arguments (represented using the type
\ciltyperef{attrarg}). Almost any expression can be used as an attribute
argument. Attributes are stored in lists sorted by the name of the attribute.
Use the function \cilvalref{typeAttrs} to retrieve the attributes of a type
and the functions \cilvalref{addAttribute} and \cilvalref{addAttributes} to
add attributes. Alternatively you can use \cilvalref{typeAddAttributes} to add
an attribute to a type (and return the new type).
