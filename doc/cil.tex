...
\begin{itemize}
\item \t{gccdir=foo} Specifies that the \t{gcc} executable that can be found
in the \t{foo} directory should be used. By default whichever version is in
the PATH is used.
\end{itemize}
...
\t{bin/cilly.pl.in}             & The \t{configure} source for a Perl script 
...
\t{lib/Merger.pm}               &  A subclass of \t{CompilerStub.pm} that can
                                  ...
                                  uses it. \\
\t{bin/patcher.pl}              & A Perl script that applies specified patches
...
is \t{bin/cilly.pl} and is quite powerful. Note that the \t{cilly.pl} script
...
\begin{verbatim}
perl bin/cilly.pl -D HAPPY_MOOD -I myincludes hello.c -o hello
\end{verbatim}
...
\begin{verbatim}
make CC="perl bin/cilly.pl" LD="perl bin/cilly.pl"
\end{verbatim}

 \t{cilly.pl} can also behave as the Microsoft Visual C compiler, if the first
 argument is \t{--mode=MSVC}:
\begin{verbatim}
perl bin/cilly.pl --mode=MSVC /D HAPPY_MOOD /I myincludes hello.c /Fe hello.exe
\end{verbatim}
...
example, 
\begin{verbatim}
perl bin/cilly.pl --logwrites -D HAPPY_MOOD -I myincludes hello.c -o hello.exe
\end{verbatim}
...
\begin{verbatim}
make CC="perl bin/cilly.pl --logwrites --merge"
\end{verbatim}

 You can even leave some files untouched:
\begin{verbatim}
make CC="perl bin/cilly.pl --logwrites --merge --leavealone=foo --leavealone=bar"
\end{verbatim}
...
\begin{verbatim}
make CC="perl bin/cilly.pl --includedir=myinclude"
\end{verbatim}
