...
If you do not use CIL or similar frameworks and want instead to use just a C
parser and analyze programs expressed as abstract-syntax trees then your
analysis will have to handle a lot of ugly corners of the language (let alone
the fact that parsing C itself is not a trivial task). See \secref{simplec}
for some examples of such extreme programs that CIL simplifies for you.
...
eliminated and function arguments with array types become pointers. (For a
more extensive list of how CIL simplifies C programs, see \sefrec{cabs2cil}.)
...
checking function calls. This means that some code that used to compile before
will not compile afterwards. For example, the following code compiles (and
yields an undefined result) but it will not compile after CIL. 
\begin{cilcode}[global]
int f(i1, i2) int i1, i2 {
  return i1 + i2;
}

int main() {
    return f(5, 6) + f(5); // f called with only one argument
}
\end{cilcode}
