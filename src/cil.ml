...
and d_line l = 
  ...
  if ls <> "" then leftflushline ++ text (printLine l) ++ line else nil
...
let printFile (out : out_channel) file =
  ...
  noBreaks := true; noAligns := true; Pretty.fastMode := true;
  assert (noBreaks := false; noAligns := false; 
          Pretty.fastMode := false; true);
